...
int adjMat[N][N] = {
    ...
    { 0, 0, 2, 2, 0, 4, 2, 3, 0, 0, 0, 0, 4, 2, 0 }, // Vertex 0
    { 0, 0, 1, 0, 0, 0, 0, 3, 0, 0, 0, 0, 4, 1, 0 }, // Vertex 1
    { 2, 1, 0, 1, 1, 0, 0, 2, 0, 3, 0, 0, 0, 0, 3 }, // Vertex 2
    { 2, 0, 1, 0, 1, 0, 2, 0, 2, 0, 0, 0, 0, 4, 2 }, // Vertex 3
    { 0, 0, 1, 1, 0, 0, 0, 5, 5, 3, 0, 5, 0, 0, 0 }, // Vertex 4
    { 4, 0, 0, 0, 0, 0, 5, 4, 0, 0, 3, 0, 0, 0, 1 }, // Vertex 5
    { 2, 0, 0, 2, 0, 5, 0, 0, 3, 2, 0, 0, 0, 0, 4 }, // Vertex 6
    { 3, 3, 2, 0, 5, 4, 0, 0, 0, 1, 4, 5, 0, 0, 0 }, // Vertex 7
    { 0, 0, 0, 2, 5, 0, 3, 0, 0, 0, 5, 3, 4, 5, 2 }, // Vertex 8
    { 0, 0, 3, 0, 3, 0, 2, 1, 0, 0, 2, 0, 0, 0, 2 }, // Vertex 9
    { 0, 0, 0, 0, 0, 3, 0, 4, 5, 2, 0, 2, 0, 0, 0 }, // Vertex 10
    { 0, 0, 0, 0, 5, 0, 0, 5, 3, 0, 2, 0, 0, 0, 0 }, // Vertex 11
    { 4, 4, 0, 0, 0, 0, 0, 0, 4, 0, 0, 0, 0, 0, 3 }, // Vertex 12
    { 2, 1, 0, 4, 0, 0, 0, 0, 5, 0, 0, 0, 0, 0, 5 }, // Vertex 13
    { 0, 0, 3, 2, 0, 1, 4, 0, 2, 2, 0, 0, 3, 5, 0 }  // Vertex 14
};
...
void depthFirst(Graph *graph, int vertex, int numVisited, int currentDistance)
{
  ...
  */
	...
	// also need to check if we are adjacent to the original start vertex (cycle condition)
	if ((numVisited == graph->numVert) && (adjMat[vertex][startVertex] > 0))
	{
		...
		numCyclesFound++;
		if ((finalDistance < bestDistance) || numCyclesFound == 1) // record if its shorter length OR the first cycle found
		{
	...
	else
	{
		...
		int vertexDeg = graph->degree[vertex]; // gets the degree of current vertex we are processing
		for (int i = 0; i < vertexDeg; i++)
		{
			int adjVert = graph->adjList[vertex][i]; // i-th vertex adjacent to vertex
			if (!visited[adjVert])
			{
				depthFirst(graph, adjVert, numVisited, currentDistance + adjMat[adjVert][vertex]);
			}
		}
	}
